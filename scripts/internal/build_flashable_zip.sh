...
ZIP_FILE_NAME="TerracottaROM_${ROM_VERSION}_$(date +%Y%m%d)_${TARGET_DEVICE}${ZIP_FILE_SUFFIX}"
...
# Add batches
EVAL "7z a -tzip -mx=9 -mmt=$(nproc) "$TMP_DIR/rom.zip" @"compressed.txt" && 7z a -tzip -mx=0 -mmt=$(nproc) "$TMP_DIR/rom.zip" @"stored.txt" "$META_INF""
